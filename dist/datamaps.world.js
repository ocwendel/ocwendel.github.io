  ...
  */
  ...
  function addContainer( element, height, width ) {
    ...
    if (this.options.responsive) {
      d3.select(this.options.element).style({'position': 'relative', 'padding-bottom': '3%'});
      d3.select(this.options.element).style({'position': 'relative', 'padding-top': '10%'});
      d3.select(this.options.element).select('svg').style({'position': 'absolute', 'width': '100%', 'height': '70%'});
  ...
  function setProjection( element, options ) {
    ...
    if ( options.scope === 'usa' ) {
      ...
      projection = d3.geo[options.projection]()
        ...
        .translate([width / 2, height / (options.projection === "mercator" ? 2 : 3)]);
    }
